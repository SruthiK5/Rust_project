...
impl Todo {
    pub fn new() -> Result<Self, String> {
        let todo_path: String = match env::var_os("TODO_PATH") {
            Some(t) => t.to_string_lossy().into_owned(),
            None => {
                let home = match env::var_os("HOME") {
                    Some(h) => h.to_string_lossy().into_owned(),
                    None => {
                        // Set default HOME value to the current working directory
                ...
                match Path::new(&legacy_todo).exists() {
                    true => legacy_todo,
                    false => {
                        // Set default TODO_PATH value if not found
                        // Modify this path as needed
                        let cwd = env::current_dir().expect("Failed to get current working directory");
                        let todo_path = cwd.join("TODO.txt"); // You can change "TODO.txt" to your preferred file name
        ...
        if let Err(err) = buf_reader.read_to_string(&mut contents) {
            eprintln!("Error reading TODO file: {:#?}", err);
            // Handle the error appropriately, e.g., return an Err from Todo::new or exit the program.
        }
   ...
   pub fn add(&self, args: &[String], _input_source: &str) {
    if args.is_empty() {
    ...
    pub fn clear(&self) {
        // Open the TODO file with a permission to truncate it (delete all content)
        let todofile = OpenOptions::new()
            .write(true)
            .truncate(true)
            .open(&self.todo_path)
            .expect("Couldn't open the todofile for clearing");
        ...
        buffer
            .write_all("".as_bytes())
            .expect("Unable to clear the todo file");
    }
